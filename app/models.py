...
class User(UserMixin, db.Model):
    ...
    code_expiration = db.Column(db.DateTime(timezone=True), nullable=True)
    reset_code = db.Column(db.String(6), nullable=True)
    reset_code_expiration = db.Column(db.DateTime(timezone=True), nullable=True)
...
class Reservation(db.Model):
    ...
    date_reservation = db.Column(db.DateTime(timezone=True), default=lambda: datetime.now(timezone.utc))
    ...
    avis = db.relationship("Avis", backref="reservation", lazy=True)
